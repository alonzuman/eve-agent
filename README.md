...
eve's current Linq adapter deduplicates incoming webhook messages in process. Cross-instance retries can still cause duplicate conversational turns. There is no claim of exactly-once message processing. The Link sandbox test uses separate encrypted purchase records, atomic execution claims and merchant reconciliation to prevent duplicate checkout submissions.

Per-user Link wallet connection is implemented through `connect_link_wallet`, `link_wallet_status`, and `disconnect_link_wallet`. `test_link_purchase` adds a fixed $5 USD no-charge Stripe sandbox checkout with a separate Link spending approval; status/resume and cancellation tools are also available. Enable the migration, wallet encryption and provisioned Stripe test key as described in [the payment integration guide](src/payments/README.md). Wallet authorization and payment credentials never enter the model's memory or general browser. Real merchant purchases, scheduled jobs, email monitoring, and password-vault integration remain unavailable. A real merchant confirmation remains necessary to pass the future flower-purchase evaluation.
