...
Call action present, then action status. Only say sent when status is sent with a messageId. On send failure use action retry for the same batch. If rendering fails, simplify the layout or find a valid real photo; fall back to concise text when needed. The tool sends its introduction, numbered captions and links, so do not duplicate them in ordinary text.
